...
export class VirshStatsComponent implements StatsComponent {
    ...
    private listener: VirshStatsListener;

    constructor(listener: VirshStatsListener) {
        this.info = new VirshStats;
...
`);
    ...
    public update_table(metadata: TableMetadata): void {
        logger.append_to_table({
            ...
            values: [[metadata.exp_name, metadata.date, metadata.info.time,
