...
function handle_vm_free_stats(stats) {
	logger.info.vm_free_total = stats.free_total;
	logger.info.vm_free_used = stats.free_used;
	logger.info.vm_free_bufcache = stats.free_buffcache;
}
...
module.exports = function(request, protocol, callbacks) {
    ...
    connection.on('message', (message) => {
		...
		else if(msg.cmd === "free_stats")         handle_vm_free_stats(msg.stats);
		else if(msg.cmd === "ip") {
			logger.info.is_medooze_connected = true;
    		callbacks.medooze_connected(connection, msg.ip);
		}
    });
