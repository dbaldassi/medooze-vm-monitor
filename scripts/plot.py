...
headers = {
    ...
    'MEMORY_USED': [1, 'b', lambda x: float(x), "Memory", "(MiB)", "cgroup memory.current"],
    ...
    'SWAP': [4, 'r', lambda x: float(x), "Memory", "(MiB)", "cgroup utilisation swap"],
    ...
    'VIRSH_AVAILABLE': [14, 'g', lambda x: float(x) / 1024., "Memory", "(MiB)", "vRAM Invité"],
    'VIRSH_SWAP_IN': [15, '', lambda x: float(x) / 1024., "Memory", "(MiB)", "VM swap in"],
    'VIRSH_SWAP_OUT': [16, 'r', lambda x: float(x) / 1024., "Memory", "(MiB)", "Swap invité"],
